...
import {
  CalendarDays, Users, UserRound, Settings, Building2, Activity, ChevronRight,
  Plus, CheckCircle2, AlertTriangle, Clock,
  Stethoscope, BarChart2, X, CalendarCheck, RefreshCw, Loader2,
...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    if (!t) { setTab("overview"); return; }
    if (t === "finance") { router.push("/hospitaladmin/finance"); return; }
    if (["overview", "appointments", "staff", "doctors", "patients", "inventory", "billing", "ipd", "departments", "enquiries", "tourism", "blogs", "reports", "settings", "profile"].includes(t)) {
      setTab(t);
    }
  }, [searchParams, router]);
  ...
  return (
    ...
    <div className="hd-center" style={tab === "billing" ? { padding: 0 } : {}}>
          ...
          <div className="hd-tbl-wrap">
            {dashboardLoading && !dashboardData ? (
              ...
              <table className="hd-tbl">
                ...
                <tbody>
                  {(dashboardData?.recentPatients ?? []).map((p: any) => {
      ...
      {tab === "billing" && (
        <BillingQueue deptName={user?.name ? `Admin ${user.name}` : "Admin"} />
      )}

    ...
    Promise.all([
      ...
      .then(([d, settingsData]) => {
        if (!d.success) {
          ...
          if (matchedCode) {
            router.push(`/login?error=${matchedCode}`);
          } else {
            router.push("/login");
          }
          return;
        ...
        setUser(d.data);
        if (settingsData && settingsData.success && settingsData.data?.settings) {
          ...
          <div className="hd-sb-logo">
            {hospitalSettings?.logo ? (
              ...
              <>
                ...
                <div><div className="hd-logo-tx">{hospitalSettings?.hospitalName || user?.hospital?.name || "AiHospitalERP"}</div><div className="hd-logo-sub">Hospital Admin</div></div>
              </>
            )}
          </div>

...
export async function POST(req: NextRequest) {
  ...
  const auth = await requireRole(req, ["HOSPITAL_ADMIN", "DOCTOR", "RECEPTIONIST", "STAFF", "SUB_DEPT_HEAD", "SUPER_ADMIN"]);
  ...
  try {
    ...
    let hid = auth.hospitalId;
    if (auth.user.role === "SUPER_ADMIN") {
      hid = (formData.get("hospitalId") as string) || req.nextUrl.searchParams.get("hospitalId") || req.nextUrl.searchParams.get("hid") || "";
    }
    ...
    else if (uploadType === "logo") folder = `hms/hospitals/${hid}/logos`;
    else if (uploadType === "letterhead") folder = `hms/hospitals/${hid}/letterheads`;
    ...
    if (isImage || isPdf) {
      ...
      uploadOptions.allowed_formats = ["jpg", "jpeg", "png", "pdf", "webp", "gif"];
    } else {

...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      // Last 9 months trends data
      appointmentsLast9Months,
      patientsLast9Months,
    ] = await Promise.all([
      // Patient stats
      ...
      // Appointment counts (fetched as list, counted in-memory)
      px.appointment.findMany({ where: { hospitalId, appointmentDate: { gte: todayStart, lte: todayEnd } }, select: { status: true } }).catch(() => []),
      ...
      // Last 9 months trends data
      px.appointment.findMany({ where: { hospitalId, appointmentDate: { gte: monthlyData[0].start } }, select: { appointmentDate: true } }).catch(() => []),
      prisma.patient.findMany({ where: { hospitalId, createdAt: { gte: monthlyData[0].start } }, select: { createdAt: true } }).catch(() => []),
    ]);

    // Aggregate today's counts in memory
    const todayAppts = todayApptsList.length;
    const completedAppts = todayApptsList.filter((a: any) => a.status === "COMPLETED").length;
    const cancelledAppts = todayApptsList.filter((a: any) => a.status === "CANCELLED").length;
    const scheduledAppts = todayApptsList.filter((a: any) => a.status === "SCHEDULED").length;
    const pendingAppts = todayApptsList.filter((a: any) => a.status === "CONFIRMED").length;

    // Aggregate monthly appointment trends in memory (0 DB roundtrips here)
    const monthlyTrends = monthlyData.map((m) => {
      const appts = appointmentsLast9Months.filter((a: any) => {
        const d = new Date(a.appointmentDate);
        return d >= m.start && d <= m.end;
      }).length;
      const patients = patientsLast9Months.filter((p: any) => {

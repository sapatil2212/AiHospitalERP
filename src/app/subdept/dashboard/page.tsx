...
import {
  LogOut, Loader2, User, Phone, Mail, Activity, LayoutDashboard, Truck,
  ...
  TestTube2, HelpCircle, CheckCircle2, AlertCircle,
  CalendarDays, FileText, TrendingUp, TrendingDown, FlaskConical,
  Plus, Edit2, Trash2, ToggleLeft, ToggleRight, IndianRupee,
  Save, Ban, ChevronDown, ChevronUp, MessageSquare, UserCheck, Eye, Download,
  BarChart2, Package, UserPlus, ArrowUpDown, FileSpreadsheet,
  FileType, AlertTriangle, CreditCard, Menu, ShoppingCart
} from "lucide-react";
...
function SubDeptDashboardContent() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    if (res.success) { setQueue(res.data.queue || []); setCompletedQueue(res.data.completedList || []); setDirectQueue(res.data.directQueue || []); }
  ...
  return (
    <>
                          ...
                          <tbody>
                            {filtered.map((d:any)=>(
                              <tr key={d.id} style={{borderBottom:"1px solid #f8fafc"}}
                                ...
                                <td style={{padding:"12px 14px"}}>
                                  <button onClick={() => { setConsentUploadTarget(d); setConsentFile(null); setConsentUrl(""); setConsentUploadMsg(""); }}
      ...
      {transferTarget && <TransferPatientModal record={transferTarget} subDepts={subDepts} onClose={() => setTransferTarget(null)} onTransfer={handleTransferPatient} />}

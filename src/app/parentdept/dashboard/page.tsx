...
import {
  Building2, Package, TrendingUp, Users, Activity,
  ...
  TestTube2, Stethoscope, FlaskConical, CheckCircle2,
} from "lucide-react";
...
function DashboardContent() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/parentdept/me", { credentials: "include" }).then(r => r.json()).catch(() => null)
    ])
      .then(([d, pd]) => {
        if (!d.success) {
          const msg = d.message || "";
...
function SubDeptDetailView({ subDeptId, onBack }: { subDeptId: string; onBack: () => void }) {
  ...
  return (
    <>
      ...
      {activeSection === "procedures" && <SubDeptProcedures procedures={data.procedures} />}
...
function SubDeptProcedures({ procedures }: { procedures: any[] }) {

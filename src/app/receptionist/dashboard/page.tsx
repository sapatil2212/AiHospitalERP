...
import {
  ClipboardList, CalendarDays, UserRound, LogOut, Bell,
  HelpCircle, Check, X, Play,
  Clock, Menu, Stethoscope
} from "lucide-react";
...
export default function ReceptionistDashboard() {
  ...
  useEffect(()=>{
    Promise.all([
      ...
      fetch("/api/config/settings",{credentials:"include"}).then(r=>r.json()).catch(()=>null)
    ])
      .then(([d, s])=>{
        if(!d.success){
          const msg = d.message || "";
          const trialCodes = ["TRIAL_EXPIRED", "SUBSCRIPTION_EXPIRED", "ACCOUNT_SUSPENDED", "ACCOUNT_CANCELLED", "HOSPITAL_DISABLED"];
          const matchedCode = trialCodes.find(code => msg.includes(code));
          router.push(matchedCode ? `/login?error=${matchedCode}` : "/login");
          return;
        }
        setUser(d.data);
        if(s && s.success && s.data?.settings) {
          setHospitalSettings(s.data.settings);
        }
        setLoading(false);
      })
        ...
        <div className="rec-logo">
          {hospitalSettings?.logo ? (
            ...
            <>
              ...
              <div><div className="rec-logo-tx">{hospitalSettings?.hospitalName || user?.hospital?.name || "AiHospitalERP"}</div><div className="rec-logo-sub">Front Desk</div></div>
            </>
          )}
        </div>

...
export default function BillingQueue({ scope, subDeptId, deptName, defaultCollectBillId, onDefaultCollectConsumed }: { scope?: "lab" | "pharmacy" | "procedure"; subDeptId?: string; deptName?: string; defaultCollectBillId?: string; onDefaultCollectConsumed?: () => void } = {}) {
  ...
  const handleCollectAndGenerate = async () => {
    ...
    try {
      ...
      // Record payment AND update the bill in a single API call
      const payRes = await api(`/api/billing/${billId}`, {
        ...
        body: JSON.stringify({
          ...
          addedCharges: collectForm.addedCharges && collectForm.addedCharges.length > 0
            ? collectForm.addedCharges.map(charge => ({
                name: charge.name,
            ...
            : undefined,
          discount: !hasExistingPayment ? (collectForm.discount || 0) : undefined,
          isGst: !hasExistingPayment ? collectForm.isGst : undefined,
          cgst: !hasExistingPayment ? collectForm.cgst : undefined,
          sgst: !hasExistingPayment ? collectForm.sgst : undefined,
          igst: !hasExistingPayment ? collectForm.igst : undefined,
        })
      });
      ...
      if (payRes.success) {
        ...
        // Auto-send email entirely in background (delayed slightly to keep transition silky smooth)

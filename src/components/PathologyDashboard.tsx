...
export default function PathologyDashboard({profile,user,activeTab,onTabChange}:{profile:any;user:any;activeTab?:string;onTabChange?:(t:string)=>void}){
  ...
  const printReport=async(rep:any)=>{
    ...
    if(hasLetterhead){
      ...
      if(!lhLogoUrl){doc.setFont("helvetica","bold");doc.setFontSize(16);doc.setTextColor(...green);doc.text(hn,lhInfoX,y+6);}
      ...
      if(!logoDataUrl){doc.setFont("helvetica","bold");doc.setFontSize(16);doc.setTextColor(...green);doc.text(hn,infoX,y+6);}
      doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor(...gray);
      let hy=y+12;
          ...
          {hospitalInfo?.letterhead?(
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"70px 24px 0",marginBottom:20}}>
              <div>
                ...
                {!hospitalInfo?.logo&&<div style={{fontSize:17,fontWeight:800,color:"#047857"}}>{hn}</div>}

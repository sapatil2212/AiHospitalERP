...
function Pagination({ page, totalItems, limit, onPageChange }: { page: number; totalItems: number; limit: number; onPageChange: (p: number) => void }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px", borderTop: "1px solid #e2e8f0", background: "#f8fafc", flexWrap: "wrap", gap: 8 }}>
      <div style={{ fontSize: 12, color: "#64748b", fontWeight: 500 }}>
        Showing <strong>{Math.min(totalItems, (page - 1) * limit + 1)}</strong> to <strong>{Math.min(totalItems, page * limit)}</strong> of <strong>{totalItems}</strong> entries
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <button 
          ...
          style={{ padding: "5px 10px", borderRadius: 6, border: "1px solid #e2e8f0", background: page === 1 ? "#f1f5f9" : "#fff", color: page === 1 ? "#94a3b8" : "#475569", fontSize: 11, fontWeight: 600, cursor: page === 1 ? "not-allowed" : "pointer" }}
        >
          Previous
        </button>
        {pageNumbers.map(p => {
          ...
          return (
            <button 
              ...
              style={{ 
                ...
                background: active ? "#0E898F" : "#fff", color: active ? "#fff" : "#475569", 
        ...
        <button 
          ...
          onClick={() => onPageChange(page + 1)} 
          style={{ padding: "5px 10px", borderRadius: 6, border: "1px solid #e2e8f0", background: page === totalPages ? "#f1f5f9" : "#fff", color: page === totalPages ? "#94a3b8" : "#475569", fontSize: 11, fontWeight: 600, cursor: page === totalPages ? "not-allowed" : "pointer" }}
        >
          Next
        </button>
      </div>
...
export default function AdminInventoryPanel({ allowDeptTransfers = true }: { allowDeptTransfers?: boolean }) {
  ...
  // ─── Loaders ───
  const loadSuppliers = useCallback(async (showSpinner?: boolean | any) => {
    const shouldShow = showSpinner !== false;
  ...
  const loadItems = useCallback(async (showSpinner?: boolean | any) => {
    const shouldShow = showSpinner !== false;
    if (shouldShow) setLoading(true);
  ...
  const loadDeptStock = useCallback(async (showSpinner?: boolean | any) => {
    const shouldShow = showSpinner !== false;
    if (shouldShow) setLoading(true);
  ...
  const loadPurchases = useCallback(async (showSpinner?: boolean | any) => {
    const shouldShow = showSpinner !== false;
    if (shouldShow) setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    const fetchOverview = async () => {
      const alreadyLoaded = loadedRef.current.overview;
      ...
      try {
        ...
        loadedRef.current.items = true;
        loadedRef.current.stock = true;
        loadedRef.current.transfers = true;
        loadedRef.current.purchases = true;
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
    ...
    const fetchItems = async () => {
      const alreadyLoaded = loadedRef.current.items;
      await loadItems(!alreadyLoaded);
      loadedRef.current.items = true;
      loadedRef.current.stock = true;
    };

    const fetchSuppliersData = async () => {
      const alreadyLoaded = loadedRef.current.suppliers;
      await loadSuppliers(!alreadyLoaded);
      loadedRef.current.suppliers = true;
    };

    const fetchTransfers = async () => {
      const alreadyLoaded = loadedRef.current.transfers;
      await loadDeptStock(!alreadyLoaded);
      loadedRef.current.transfers = true;
    };

    const fetchPurchasesData = async () => {
      const alreadyLoaded = loadedRef.current.purchases;
      await loadPurchases(!alreadyLoaded);
      loadedRef.current.purchases = true;
    };

    if (isOverview) {
      fetchOverview();
    } else if (isItems || isStock) {
      fetchItems();
    } else if (isSuppliers) {
      fetchSuppliersData();
    } else if (isTransfers) {
    ...
    >
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 20, background: "#fff", borderBottom: "1px solid #e2e8f0", display: "flex", flexWrap: "wrap", gap: 0, padding: "6px 8px", flexShrink: 0 }}>
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => { setTab(t.id); setSearchVal(""); setSearch(""); }} style={{
      ...
      {!loading && tab === "items" && (() => {
        ...
        const paginatedItems = sortedItems.slice((itemsPage - 1) * ITEMS_PER_PAGE, itemsPage * ITEMS_PER_PAGE);
        ...
        return (<>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 9, padding: "8px 13px", flex: 1, minWidth: 200 }}>
              ...
              <input style={{ border: "none", background: "none", outline: "none", fontSize: 13, width: "100%", color: "#1e293b" }} placeholder="Search items, categories..." value={searchVal} onChange={e => setSearchVal(e.target.value)} />
            </div>
          ...
          <Pagination page={itemsPage} totalItems={sortedItems.length} limit={ITEMS_PER_PAGE} onPageChange={setItemsPage} />
        </>);
      })()}
      ...
      {!loading && tab === "stock" && (() => {
        ...
        const paginatedStock = stockItems.slice((stockPage - 1) * ITEMS_PER_PAGE, stockPage * ITEMS_PER_PAGE);
        ...
        return (<>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 9, padding: "8px 13px", flex: 1, minWidth: 200 }}>
              <Search size={13} color="#94a3b8" />
              <input style={{ border: "none", background: "none", outline: "none", fontSize: 13, width: "100%", color: "#1e293b" }} placeholder="Search items by name, category..." value={searchVal} onChange={e => setSearchVal(e.target.value)} />
            </div>
          ...
          <Pagination page={stockPage} totalItems={stockItems.length} limit={ITEMS_PER_PAGE} onPageChange={setStockPage} />
        </>);
      })()}
      ...
      {!loading && tab === "suppliers" && (() => {
        ...
        const paginatedSupp = sortedSupp.slice((suppPage - 1) * ITEMS_PER_PAGE, suppPage * ITEMS_PER_PAGE);
        ...
        return (<>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 9, padding: "8px 13px", flex: 1, minWidth: 200 }}>
              <Search size={13} color="#94a3b8" />
              <input style={{ border: "none", background: "none", outline: "none", fontSize: 13, width: "100%", color: "#1e293b" }} placeholder="Search suppliers..." value={searchVal} onChange={e => setSearchVal(e.target.value)} />
            </div>
          ...
          <Pagination page={suppPage} totalItems={sortedSupp.length} limit={ITEMS_PER_PAGE} onPageChange={setSuppPage} />
        </>);
      })()}
      ...
      {!loading && tab === "purchases" && (() => {
        ...
        const paginatedPurch = sortedPurch.slice((purchPage - 1) * ITEMS_PER_PAGE, purchPage * ITEMS_PER_PAGE);
        ...
        return (<>
          ...
          <Pagination page={purchPage} totalItems={sortedPurch.length} limit={ITEMS_PER_PAGE} onPageChange={setPurchPage} />
        </>);
      })()}
      ...
      {allowDeptTransfers && !loading && tab === "transfers" && (() => {
        ...
        const paginatedTrans = sortedTrans.slice((transPage - 1) * ITEMS_PER_PAGE, transPage * ITEMS_PER_PAGE);
        ...
        return (<>
          ...
          <Pagination page={transPage} totalItems={sortedTrans.length} limit={ITEMS_PER_PAGE} onPageChange={setTransPage} />
        </>);
      })()}

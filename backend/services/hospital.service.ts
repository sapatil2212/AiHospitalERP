...
export const getSuperAdminDashboardStats = async () => {
  ...
  // Auto-expire trials that have passed their end date (asynchronously in background)
  (prisma as any).hospital.updateMany({
    where: {
  ...
  }).catch((err: any) => console.error("Error auto-expiring trials in background:", err));

  // Auto-expire active subscriptions that have passed their end date (asynchronously in background)
  (prisma as any).hospital.updateMany({
    where: {
  ...
  }).catch((err: any) => console.error("Error auto-expiring active subscriptions in background:", err));
